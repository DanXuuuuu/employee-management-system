  ...
  try {
    ...
    for (const emp of employees) {
      ...
      const docs = await Document.find({
        owner: emp.user._id, // Document.owner = userId
        type: { $in: steps },
      }).sort({ createdAt: 1 });
      ...
      for (const step of steps) {
        ...
        // Approved: if all docs Approved，result shows all approved
...
// POST /api/hr/visa/:employeeId/send-reminder
exports.sendVisaReminder = async (req, res, next) => {
  try {
    const { userId } = req.params;

    const employee = await Employee.findOne({ user: userId }).populate('user', 'username email');
